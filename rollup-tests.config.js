...
function normalizeGeneratedTestInputs() {
  return {
    name: 'normalize-generated-test-inputs',
    load(id) {
      const normalizedId = id.replace(/\\/g, '/');
      if (!normalizedId.endsWith('build/scripts/test-inputs.js')) {
        return null;
      }

      // `@hypothesis/frontend-build` generates native Windows path separators
      // here. In JS string imports, sequences such as `\x` are parsed as
      // escapes, so normalize to URL-style separators before Rollup parses.
      return readFileSync(id, 'utf8').replace(/\\/g, '/');
    },
  };
}
